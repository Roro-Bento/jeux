...
(function (global) {
  ...
  function updateHud(v) {
    $('#m-moves').innerHTML = '<b>' + v.moves + '</b><span>pas</span>';
    $('#m-traps').innerHTML = '<b class="' + (v.trapsHit ? 'bad' : '') + '">' + v.trapsHit + '</b><span>pièges</span>';
  ...
  function walkerHtml(v) {
    return hudHtml() +
      '<section class="panel" id="m-panel">' +
        ...
        '<div class="dpad">' +
          '<button class="dp dp-n" data-d="n"><span>▲</span><i>nord</i></button>' +
          '<button class="dp dp-w" data-d="w"><span>◀</span><i>ouest</i></button>' +
          '<button class="dp dp-e" data-d="e"><span>▶</span><i>est</i></button>' +
          '<button class="dp dp-s" data-d="s"><span>▼</span><i>sud</i></button>' +
        '</div>' +
  ...
  function guideHtml(v) {
    return hudHtml() +
      '<section class="panel" id="m-panel">' +
        ...
        '<div class="map-wrap"><div class="maze" id="m-map"></div></div>' +
        '<div class="legend" id="m-legend"></div>' +
        '<div class="say" id="m-say"></div>' +
        '<div class="center"><button class="linky" id="m-clear">effacer mes marques</button></div>' +
        '<p class="mod-hint">Tu ne sais pas où il est. Fais-lui décrire les murs et les balises, ' +
        'clique sur une case pour noter ton hypothèse.</p>' +
      '</section>' +
      '<div class="center"><button class="linky" id="m-quit">quitter la partie</button></div>';
  }
  ...
  function mazeHtml(m, opts) {
    ...
    var out = '<div class="maze" style="--n:' + m.w + '">';
    for (var i = 0; i < m.w * m.h; i++) {
      ...
      out += '<div class="' + cls.join(' ') + '" data-i="' + i + '">' + inner + '</div>';
    }
    return out + '</div>';
  }
  ...
  function updateGuide(v) {
    ...
    if (sig !== L.mapSig) {
      L.mapSig = sig;
      $('#m-map').outerHTML = mazeHtml(m, { pins: L.pins });
  ...
  global.Room.init({
    ...
    bind: function (key, v) {
      ...
      if (key === 'walk') {
        $$('.dp').forEach(function (el) { el.onclick = function () { move(el.dataset.d); }; });
      } else if (key === 'guide') {
        L.pins = {};
        var clear = $('#m-clear');
        if (clear) clear.onclick = function () { L.pins = {}; L.mapSig = ''; updateGuide(L.view); };
