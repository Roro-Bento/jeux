...
(function (global) {
  ...
  function updateBoard(v) {
    ...
    $('#g-round').textContent = 'Manche ' + v.round + '/' + v.roundCount;
    var pts = v.scores[1] + v.scores[2];
    $('#g-score').textContent = pts + ' pt' + (pts > 1 ? 's' : '');
    ...
    $('#g-pips').innerHTML = pips;

    $('#g-role').innerHTML = v.isGiver
    ...
    $('#g-hearts').innerHTML = hearts + '<span class="hearts-cnt">' + api.plural(v.attemptsLeft, 'essai') + '</span>';
