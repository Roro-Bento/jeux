...
(function (global) {
  ...
  var RENDER = {
    ...
    sequence: {
      ...
      frame: function (st) {
        ...
        $$('.pad').forEach(function (el) { el.classList.toggle('lit', el.dataset.c === lit); });
      }
    },
  ...
  function updateDefuser(v) {
    D.view = v;
    $('#b-widgets').innerHTML = widgetsHtml(v);
    $('#b-strikes').innerHTML = strikesHtml(v);
    $('#b-progress').innerHTML = '<b>' + v.solvedCount + '/' + v.moduleCount + '</b>' +
