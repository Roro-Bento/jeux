...
(function (global) {
  ...
  function tilesHtml(v) {
    return '<div class="tiles">' + v.modules.map(function (m, i) {
      return '<button type="button" class="tile' + (m.solved ? ' done' : '') +
        '" data-act="open" data-i="' + i + '">' +
  ...
  function moduleHtml(v, i) {
    ...
    return '<div class="mod-head">' +
        '<button type="button" class="linky" id="b-back" data-act="back">◀ boîtier</button>' +
    ...
    fils: {
      html: function (st) {
        return '<div class="mod-fils"><div class="wires">' + st.wires.map(function (c, k) {
          ...
          return '<button type="button" class="wire' + (cut ? ' cut' : '') +
            '" data-act="cut" data-k="' + k + '"' + (cut ? ' disabled' : '') + '>' +
      ...
      act: function (el, i) { act(i, { type: 'cut', i: parseInt(el.dataset.k, 10) }); }
    },
    ...
    bouton: {
      html: function (st) {
        return '<div class="mod-bouton">' +
          ...
          '<button type="button" class="bigbutton c-' + st.color + (st.holding ? ' held' : '') + '" id="bb">' +
    ...
    sequence: {
      html: function (st) {
        var pads = global.Modules.tables.SEQ_COLORS.map(function (c) {
          return '<button type="button" class="pad p-' + c + '" data-act="pad" data-c="' + c + '">' +
            '<span>' + c + '</span></button>';
        }).join('');
      ...
      act: function (el, i) { act(i, { type: 'press', color: el.dataset.c }); },
      frame: function (st) {
      ...
      html: function (st) {
        var wheels = st.cols.map(function (col, c) {
          return '<div class="wheel">' +
            '<button type="button" class="wbtn" data-act="cycle" data-c="' + c + '" data-d="-1">▲</button>' +
            '<div class="wletter">' + col[st.pos[c]] + '</div>' +
            '<button type="button" class="wbtn" data-act="cycle" data-c="' + c + '" data-d="1">▼</button>' +
            '</div>';
        }).join('');
        return '<div class="mod-pass"><div class="wheels">' + wheels + '</div>' +
          '<button type="button" class="btn btn-primary" id="pw-ok" data-act="pw-ok">Valider</button>' +
          '<p class="mod-hint">Fais défiler une molette pour lire ses six lettres.</p></div>';
      },
      act: function (el, i) {
        if (el.dataset.act === 'pw-ok') act(i, { type: 'submit' });
        else act(i, { type: 'cycle', col: parseInt(el.dataset.c, 10), dir: parseInt(el.dataset.d, 10) });
      }
    },
    ...
    morse: {
      html: function (st) {
        return '<div class="mod-morse">' +
          '<div class="morse-lamp" id="morse-led"><i></i></div>' +
          '<div class="morse-caption" id="morse-cap"></div>' +
          '<div class="freq-row">' +
            '<button type="button" class="wbtn" data-act="freq" data-d="-1">◀</button>' +
            '<div class="freq"><b>' + st.freqs[st.index] + '</b> MHz</div>' +
            '<button type="button" class="wbtn" data-act="freq" data-d="1">▶</button>' +
          '</div>' +
          '<button type="button" class="btn btn-primary" id="morse-send" data-act="send">Transmettre</button>' +
          '<button type="button" class="linky" id="morse-speed" data-act="speed">' +
            (D.morseSlow ? 'vitesse normale' : 'ralentir le signal') + '</button>' +
          '<p class="mod-hint"><b>Rond</b> = signal court, <b>barre</b> = signal long. ' +
          'Une pause marque la fin de chaque lettre.</p></div>';
      },
      act: function (el, i) {
        var what = el.dataset.act;
        if (what === 'send') { act(i, { type: 'send' }); return; }
        if (what === 'speed') {
          D.morseSlow = !D.morseSlow;
          D.bodySig = '';
          updateDefuser(D.view);
          return;
        }
        act(i, { type: 'freq', dir: parseInt(el.dataset.d, 10) });
      },
      frame: function (st) {
        var lamp = $('#morse-led');
        if (!lamp) return;
        var unit = D.morseSlow ? MORSE_UNIT_SLOW : MORSE_UNIT;
        var key = st.signal + '@' + unit;
        var tl = D.morseCache[key] || (D.morseCache[key] = morseTimeline(st.signal, unit));
        var kind = morseAt(tl, Date.now() % tl.total);
        if (lamp.__kind === kind) return;
        lamp.__kind = kind;

        lamp.classList.toggle('on', kind === 'dot' || kind === 'dash');
        lamp.classList.toggle('dash', kind === 'dash');

        var cap = $('#morse-cap');
        if (cap) {
          cap.className = 'morse-caption' + (kind === 'endgap' ? ' restart' : kind === 'lettergap' ? ' sep' : '');
          cap.textContent = kind === 'endgap' ? '⟳ le message reprend'
            : kind === 'lettergap' ? '· lettre suivante ·' : '';
        }
      }
    },
    ...
    symboles: {
      html: function (st) {
        return '<div class="mod-sym"><div class="symkeys">' + st.keys.map(function (s) {
          ...
          return '<button type="button" class="symkey' + (done ? ' done' : '') +
            '" data-act="key" data-s="' + esc(s) + '">' + s + '</button>';
        }).join('') + '</div>' +
      ...
      act: function (el, i) { act(i, { type: 'key', sym: el.dataset.s }); }
    }
  };
  ...
  /* Rythme du morse, en « unités ». Les proportions sont volontairement plus
   * lisibles que le morse réel : un trait dure 3 fois un point, et la pause
   * entre deux lettres dure 4 unités au lieu de 3, pour qu'on la voie passer.
   *   point 1 · trait 3 · silence dans la lettre 1 · entre lettres 4 · fin 9
   */
  var MORSE_UNIT = 300;         // millisecondes par unité
  var MORSE_UNIT_SLOW = 500;    // mode « ralenti »

  function morseTimeline(signal, unit) {
    var seg = [], total = 0;
    function push(kind, units) {
      var d = units * unit;
      if (d > 0) { seg.push({ kind: kind, d: d }); total += d; }
    }
    signal.split(' ').forEach(function (letter, li, arr) {
      letter.split('').forEach(function (sym, si) {
        push(sym === '.' ? 'dot' : 'dash', sym === '.' ? 1 : 3);
        if (si < letter.length - 1) push('gap', 1);          // dans la lettre
      });
      push(li === arr.length - 1 ? 'endgap' : 'lettergap', li === arr.length - 1 ? 9 : 4);
    });
    return { seg: seg, total: total };
  }

  function morseAt(tl, t) {
    var acc = 0;
    for (var k = 0; k < tl.seg.length; k++) {
      acc += tl.seg[k].d;
      if (t < acc) return tl.seg[k].kind;
    }
    return 'gap';
  }
    ...
    return '<div class="hud bomb-hud">' +
      ...
      '<section class="panel manual">' +
        ...
        '<nav class="man-nav" id="man-nav">' + Manual.sections.map(function (s) {
          return '<button type="button" data-act="sect" data-s="' + s.id + '">' +
            s.emoji + ' ' + esc(s.title) + '</button>';
        }).join('') + '</nav>' +
  ...
  function onAct(el) {
    ...
    if (what === 'sect') {                       // onglet du manuel
      D.section = el.dataset.s;
      D.manSig = '';
      updateExpert(v);
      return;
    }
    if (what === 'open') {                       // ouvrir un module
      D.open = parseInt(el.dataset.i, 10);
      D.bodySig = '';
      updateDefuser(v);
      return;
    }
    if (what === 'back') {                       // revenir au boîtier
      D.open = null;
      D.bodySig = '';
      updateDefuser(v);
      return;
    }
    if (D.open == null || !v.modules) return;    // les gestes propres à un module
    var m = v.modules[D.open];
    if (!m || m.solved) return;
    var r = RENDER[m.id];
    if (r && r.act) r.act(el, D.open);
  }

  function bindCommon() {
    var q = $('#b-quit');
    if (q) q.onclick = api.leave;
    api.onTap($('#b-panel') || $('.panel'), onAct);
