...
(async () => {
  ...
  const labels = await B.$$eval('.mf-cols span, .mf-rows span', els => els.map(e => e.textContent));
  if (labels.join('') !== 'ABCDEFGHI123456789') fail('les repères de colonnes/lignes manquent : ' + labels.join(''));
  const gridBg = await B.evaluate(() => getComputedStyle(document.querySelector('.maze')).backgroundImage);
  if (!/linear-gradient/.test(gridBg)) fail('le quadrillage des cases n’est pas dessiné');
  ok('plan quadrillé, colonnes A→I et lignes 1→9');
