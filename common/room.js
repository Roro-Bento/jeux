...
(function (global) {
  ...
  function onMessage(role, m) {
    if (!m || !m.t) return;
    /* Canal direct entre les deux joueurs, qui court-circuite le moteur.
     * Sert à ce qui doit arriver en continu et n'a pas à passer par l'état de
     * la partie — le trait de crayon du Pictionary, typiquement. */
    if (m.t === 'peer') { if (def.onPeer) def.onPeer(m.m, role); return; }
    if (role === 'host') {
      if (m.t === 'hello') {
  ...
  function sendPeer(obj) {
    if (A.conn) A.conn.send({ t: 'peer', m: obj });
  }
  ...
  /* ------------------------------------------------------------- les clics
   * Un seul écouteur posé sur un conteneur stable, et les éléments cliquables
   * sont repérés par [data-act]. Deux raisons :
   *  - les panneaux sont reconstruits en cours de partie ; avec un écouteur
   *    par bouton, un rebuild mal placé fait disparaître la réactivité ;
   *  - certains navigateurs (Opera et ses gestes à la souris, par exemple)
   *    n'émettent pas « click » si le pointeur bouge de quelques pixels entre
   *    l'appui et le relâchement. On écoute donc aussi « pointerup », en
   *    ignorant le « click » qui suivrait, pour ne pas déclencher deux fois.
   */
  function onTap(container, handler) {
    if (!container || container.__tap) return;
    container.__tap = true;

    var downEl = null, downX = 0, downY = 0, muteClickUntil = 0;

    function hit(e) {
      var el = e.target && e.target.closest ? e.target.closest('[data-act]') : null;
      return el && container.contains(el) && !el.disabled ? el : null;
    }
    ...
    container.addEventListener('pointerup', function (e) {
      var el = hit(e);
      if (!el || el !== downEl) return;
      if (Math.abs(e.clientX - downX) > 12 || Math.abs(e.clientY - downY) > 12) return;
      muteClickUntil = Date.now() + 500;
      handler(el, e);
    });

    // clavier et navigateurs qui n'ont pas d'événements pointer
    container.addEventListener('click', function (e) {
      if (Date.now() < muteClickUntil) return;
      var el = hit(e);
      if (el) handler(el, e);
    });
  }
